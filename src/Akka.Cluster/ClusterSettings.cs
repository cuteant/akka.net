...
namespace Akka.Cluster
{
    ...
    public sealed class ClusterSettings
    {
        ...
        public ClusterSettings(Config config, string systemName)
        {
            ...
            var useDownRemoval = clusterConfig.GetString(key, "").ToLowerInvariant();
            DownRemovalMargin =
                (
                    string.Equals(useDownRemoval, "off") ||
                    string.Equals(useDownRemoval, "false") ||
                    string.Equals(useDownRemoval, "no")
                ) ? TimeSpan.Zero :
