...
namespace Akka.Remote
{
    ...
    /// INTERNAL API.
    ///
    /// The remote transport is responsible for sending and receiving messages. Each transport has an
    /// address, which it should provide in Serialization.CurrentTransportInformation (thread-local)
    /// while serializing ActorReferences (which might also be part of messages). This address must
    /// be available (i.e. fully initialized) by the time the first message is received or when the
    /// Start() method returns, whichever happens first.
    /// </summary>
    [InternalApi]
    public abstract class RemoteTransport
    {
        ...
        /// <summary>Addresses to be used in <see cref="RootActorPath"/> of refs generated for this transport.</summary>
        public abstract ISet<Address> Addresses { get; }

        /// <summary>The default transport address of the <see cref="ActorSystem"/>. This is the listen
        /// address of the default transport.</summary>
        public abstract Address DefaultAddress { get; }

        /// <summary>When true, some functionality will be turned off for security purposes</summary>
        protected bool UseUntrustedMode { get; set; }

        /// <summary>TBD</summary>
        public bool logRemoteLifeCycleEvents { get; protected set; }

        /// <summary>A logger that can be used to log issues that may occur</summary>
        public ILoggingAdapter Log { get; protected set; }

        /// <summary>Start up the transport, i.e. enable incoming connections</summary>
        public abstract void Start();

        /// <summary>Shuts down the remoting</summary>
        /// <returns>A Task that can be waited on until shutdown is complete</returns>
        public abstract Task Shutdown();

        /// <summary>Sends the given message to the recipient, supplying <paramref name="sender"/> if any.</summary>
        /// <param name="message">TBD</param>
        /// <param name="sender">TBD</param>
        /// <param name="recipient">TBD</param>
        public abstract void Send(object message, IActorRef sender, RemoteActorRef recipient);

        /// <summary>Sends a management command to the underlying transport stack. The call returns with a
        /// Task that indicates if the command was handled successfully or dropped.</summary>
        ...
        /// <summary>Resolves the correct local address to be used for contacting the given remote address.</summary>
    ...
    /// <summary>This exception is thrown when a general failure within a <see cref="RemoteTransport"/>
    /// occurs, such as the inability to start, wrong configuration, etc...</summary>
