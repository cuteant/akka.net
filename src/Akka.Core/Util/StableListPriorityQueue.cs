...
namespace Akka.Util
{
    ...
    {
        private readonly struct WrappedEnvelope
        {
            public WrappedEnvelope(in Envelope envelope, int sequenceNumber)
            {
                Envelope = envelope;
        ...
        private class WrappedEnvelopeComparator
        {
            ...
            public int Compare(in WrappedEnvelope x, in WrappedEnvelope y)
            {
        ...
        public bool IsConsistent()
        {
            ...
            if (0u >= (uint)_data.Count) return true;
            var li = dataCount - 1; // last index
            for (var pi = 0; pi < dataCount; ++pi) // each parent index
            {
