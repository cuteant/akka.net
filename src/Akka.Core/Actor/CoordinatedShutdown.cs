...
namespace Akka.Actor
{
    ...
    public sealed class CoordinatedShutdownExtension : ExtensionIdProvider<CoordinatedShutdown>
    {
        ...
        public override CoordinatedShutdown CreateExtension(ExtendedActorSystem system)
        {
            ...
            if (conf.IsNullOrEmpty()) { AkkaThrowHelper.ThrowConfigurationException_Akka_Coordinated_Shutdown_Config_Cannot_Be_Empty(); }
    ...
    public sealed class CoordinatedShutdown : IExtension
    {
        ...
        private readonly ConcurrentDictionary<string, ImmutableList<(string, IRunnableTask<Done>)>> _tasks = new ConcurrentDictionary<string, ImmutableList<(string, IRunnableTask<Done>)>>(StringComparer.Ordinal);
        ...
        public void AddTask(string phase, string taskName, IRunnableTask<Done> task)
        {
            ...
            {
                if (!_tasks.TryAdd(phase, ImmutableList<(string, IRunnableTask<Done>)>.Empty.Add((taskName, task))))
        ...
        {
            if (config.IsNullOrEmpty()) { AkkaThrowHelper.ThrowConfigurationException_Invalid_phase_configuration(); }
