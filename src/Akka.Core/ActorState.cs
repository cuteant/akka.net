...
namespace Akka.Actor
{
    ...
    {
        private readonly Dictionary<IActorRef, Option<object>> _watching = new Dictionary<IActorRef, Option<object>>(ActorRefComparer.Instance);
        private readonly HashSet<IActorRef> _watchedBy = new HashSet<IActorRef>(ActorRefComparer.Instance);
        private readonly Dictionary<IActorRef, Option<object>> _terminatedQueue = new Dictionary<IActorRef, Option<object>>(ActorRefComparer.Instance); //terminatedqueue should never be used outside the message loop
