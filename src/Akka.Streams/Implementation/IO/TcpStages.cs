            ...
            private void Receive((IActorRef, object) args)
            {
                ...
                switch (msg)
                {
                    ...
                    case Tcp.Connected connected:
                        ...
                        break;
                    case Tcp.Unbind _:
                        if (!IsClosed(_stage._out) && !(_listener is null))
                            TryUnbind();
                        break;
                    case Tcp.Unbound _:
                        UnbindCompleted();
        ...
        internal sealed class TcpStreamLogic : GraphStageLogic
        {
            ...
            public override void PreStart()
            {
                ...
                if (_role is Inbound inbound)
                {
                    ...
                    GetStageActor(_connectedReceiveFunc).Watch(_connection);
            ...
            {
                void LocalReceive((IActorRef, object) args)
                {
                    var sender = args.Item1;
                    var msg = args.Item2;
                    switch (msg)
                    {
                        ...
                        case Tcp.Connected connected:
                            ...
                            StageActor.Become(_connectedReceiveFunc);
