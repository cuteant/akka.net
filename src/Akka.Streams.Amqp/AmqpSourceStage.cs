        ...
        private class AmqpSourceStageLogic : AmqpConnectorLogic
        {
            ...
            private class DefaultConsumer : DefaultBasicConsumer
            {
                    ...
                    var committableMessage =
                        new CommittableIncomingMessage(
                            ...
                            ack: multiple =>
                            {
                                ...
                                _commitCallback.Handle(new AckArguments(message.Envelope.DeliveryTag, multiple, promise));
                                return promise.Task;
                            },
                            nack: (multiple, requeue) =>
                            {
                                var promise = new TaskCompletionSource<Done>();
                                _commitCallback.Handle(new NackArguments(message.Envelope.DeliveryTag, multiple, requeue, promise));
                                return promise.Task;
                            }
                        );
                ...
                public override void HandleBasicCancel(string consumerTag)
                {
                    ...
                    _shutdownCallback?.Handle(null);
                }

                public override void HandleModelShutdown(object model, ShutdownEventArgs reason)
                {
                    _shutdownCallback?.Handle(reason);
                }
            }
        }
    }
