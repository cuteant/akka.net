...
namespace Akka.IO
{
    ...
    internal abstract class TcpConnection : ActorBase, IRequiresMessageQueue<IUnboundedMessageQueueSemantics>
    {
...
#if DEBUG
            _traceLogging = tcp.Settings.TraceLogging && Log.IsDebugEnabled;
#endif
...
#if DEBUG
                        if (_traceLogging) Log.Debug("Configured registration timeout of [{0}] expired, stopping", Tcp.Settings.RegisterTimeout);
#endif
                        Context.Stop(Self);
                        return true;
        ...
        protected void CompleteConnect(IActorRef commander, IEnumerable<Inet.SocketOption> options)
        {
            ...
            {
                if (_traceLogging) { Log.Debug("Could not enable TcpNoDelay: {0}", e.Message); }
            }
#else
        ...
        {
#if DEBUG
            if (_traceLogging) { Log.Debug("Closing connection due to IO error {0}", exception); }
#endif
