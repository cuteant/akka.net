//-----------------------------------------------------------------------
// <copyright file="BufferPool.cs" company="Akka.NET Project">
...
namespace Akka.IO.Buffers
{
    ...
    internal sealed class DirectBufferPool : IBufferPool
    {
        ...
        private void AllocateSegment()
        {
            lock (_syncRoot)
            {
                if ((uint)_segments.Count >= (uint)_maxSegmentCount)
                {
                    ThrowBufferPoolAllocationException(_maxSegmentCount, _segmentSize);
                }
        ...
        private static void ThrowBufferPoolAllocationException(int maxSegmentCount, int segmentSize)
        {
            throw GetException();
            BufferPoolAllocationException GetException()
            {
                return new BufferPoolAllocationException($"Buffer pool cannot allocate more segments. A maximum capacity of {maxSegmentCount * segmentSize} bytes has been reached.");
            }
        }
        ...
        public ByteBuffer Rent()
        {
            ...
            throw GetBufferPoolAllocationException();
        }

        [MethodImpl(MethodImplOptions.NoInlining)]
        private static BufferPoolAllocationException GetBufferPoolAllocationException()
        {
            return new BufferPoolAllocationException($"Buffer pool failed to return byte buffer after {Retries} retries made.");
        }
        ...
        public IEnumerable<ByteBuffer> Rent(int minimumSize)
        {
            ...
            try
            {
                ...
                throw GetBufferPoolAllocationException(minimumSize);
            }
            catch
            {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static BufferPoolAllocationException GetBufferPoolAllocationException(int minimumSize)
        {
            return new BufferPoolAllocationException($"Couldn't allocate enough byte buffer to fill the tolal requested size of {minimumSize} bytes");
        }
