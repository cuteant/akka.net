    ...
    public class TestSerializer : SerializerWithStringManifest
    {
        ...
        public override object FromBinary(byte[] bytes, string manifest)
        {
            ...
            switch (manifest)
            {
                case "A":
                    ...
                    var fromStr = parts[0];
                    var toStr = parts[1];
                    var from = _system.Provider.ResolveActorRef(fromStr);
                    var to = _system.Provider.ResolveActorRef(toStr);
        ...
        private void VerifyTransportInfo()
        {
            switch (Akka.Serialization.Serialization.CurrentTransportInformation)
            {
                ...
                case Information t:
                    if (!t.System.Equals(_system))
                        throw new InvalidOperationException($"wrong system in CurrentTransportInformation, {t.System} != {_system}");
                    if (t.Address != _system.Provider.DefaultAddress)
                        throw new InvalidOperationException(
                            $"wrong address in CurrentTransportInformation, {t.Address} != {_system.Provider.DefaultAddress}");
                    break;
            }
        }
    }
